...
@section('content')
<div class="container">
    <h4>Kết quả thống kê sử dụng phòng máy</h4>
    <a href="{{ route('admin.statistics.index') }}" class="btn btn-secondary mt-3">Quay lại</a>
    <form action="{{ route('admin.statistics.lab.export') }}" method="GET" class="d-inline">
...
<script>
    ...
    const chart = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: {!! json_encode($data->map(fn($d) => $d->lab->name)) !!},
            datasets: [{
                label: 'Tổng số buổi sử dụng',
                data: {!! json_encode($data->map(fn($d) => $d->total_sessions)) !!},
